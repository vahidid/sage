...
func generateViaResponses(provider, url, apiKey, model, diff string, extraHeaders map[string]string) (string, error) {
	body, err := json.Marshal(newOpenAICompatibleResponsesRequest(model, structuredCommitMessages(diff)))
	if err != nil {
	...
	if err != nil {
		return "", fmt.Errorf("%s returned invalid structured output: %w", provider, err)
	}
	return message, nil
}
